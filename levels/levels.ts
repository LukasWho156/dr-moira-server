import { LevelDefinition } from '../model/level';
import allergicReaction from './allergic-reaction.lvl.json' assert { type: "json" };
import antibodies from './antibodies.lvl.json' assert { type: "json" };
import anticoagulant from './anticoagulant.lvl.json' assert { type: "json" };
import antihistaminic from './antihistaminic.lvl.json' assert { type: "json" };
import antimicrobialResistance from './antimicrobial-resistance.lvl.json' assert { type: "json" };
import atlas from './atlas.lvl.json' assert { type: "json" };
import autoimmuneDisease from './autoimmune-disease.lvl.json' assert { type: "json" };
import axon from './axon.lvl.json' assert { type: "json" };
import bloodBrainBarrier from './blood-brain-barrier.lvl.json' assert { type: "json" };
import bloodClot from './blood-clot.lvl.json' assert { type: "json" };
import bloodPressureRegulation from './blood-pressure-regulation.lvl.json' assert { type: "json" };
import callostomy from './callostomy.lvl.json' assert { type: "json" };
import capillaryForces from './capillary-forces.lvl.json' assert { type: "json" };
import castleDraft from './castle-draft.lvl.json' assert { type: "json" };
import cellWall from './cell-wall.lvl.json' assert { type: "json" };
import chainReactionIi from './chain-reaction-ii.lvl.json' assert { type: "json" };
import chainReaction from './chain-reaction.lvl.json' assert { type: "json" };
import circulation from './circulation.lvl.json' assert { type: "json" };
import comorbidity from './comorbidity.lvl.json' assert { type: "json" };
import constipation from './constipation.lvl.json' assert { type: "json" };
import corona from './corona.lvl.json' assert { type: "json" };
import dialysis from './dialysis.lvl.json' assert { type: "json" };
import diplopia from './diplopia.lvl.json' assert { type: "json" };
import endoscopicSurgery from './endoscopic-surgery.lvl.json' assert { type: "json" };
import enzymeActivator from './enzyme-activator.lvl.json' assert { type: "json" };
import fallingSickness from './falling-sickness.lvl.json' assert { type: "json" };
import gastricTorsion from './gastric-torsion.lvl.json' assert { type: "json" };
import headache from './headache.lvl.json' assert { type: "json" };
import helicobacterPylori from './helicobacter-pylori.lvl.json' assert { type: "json" };
import influenza from './influenza.lvl.json' assert { type: "json" };
import inhibitionI from './inhibition-i.lvl.json' assert { type: "json" };
import inhibitionIi from './inhibition-ii.lvl.json' assert { type: "json" };
import isletsOfLangerhans from './islets-of-langerhans.lvl.json' assert { type: "json" };
import kidneyStone from './kidney-stone.lvl.json' assert { type: "json" };
import lockedInSyndrome from './locked-in-syndrome.lvl.json' assert { type: "json" };
import longTermStudy from './long-term-study.lvl.json' assert { type: "json" };
import lymphDrainage from './lymph-drainage.lvl.json' assert { type: "json" };
import medicationPlan from './medication-plan.lvl.json' assert { type: "json" };
import micellae from './micellae.lvl.json' assert { type: "json" };
import migraine from './migraine.lvl.json' assert { type: "json" };
import minimalDosageIi from './minimal-dosage-ii.lvl.json' assert { type: "json" };
import minimalDosageIii from './minimal-dosage-iii.lvl.json' assert { type: "json" };
import minimalDosageIv from './minimal-dosage-iv.lvl.json' assert { type: "json" };
import minimalDosage from './minimal-dosage.lvl.json' assert { type: "json" };
import motorSkills from './motor-skills.lvl.json' assert { type: "json" };
import multimorbidity from './multimorbidity.lvl.json' assert { type: "json" };
import muscleRelaxant from './muscle-relaxant.lvl.json' assert { type: "json" };
import myelinSheath from './myelin-sheath.lvl.json' assert { type: "json" };
import osSacrum from './os-sacrum.lvl.json' assert { type: "json" };
import overdose from './overdose.lvl.json' assert { type: "json" };
import pediatricDisease from './pediatric-disease.lvl.json' assert { type: "json" };
import perestalsis from './perestalsis.lvl.json' assert { type: "json" };
import phospholipidBilayer from './phospholipid-bilayer.lvl.json' assert { type: "json" };
import phytomedicine from './phytomedicine.lvl.json' assert { type: "json" };
import pitfalls from './pitfalls.lvl.json' assert { type: "json" };
import plateaus from './plateaus.lvl.json' assert { type: "json" };
import pneumothorax from './pneumothorax.lvl.json' assert { type: "json" };
import pons from './pons.lvl.json' assert { type: "json" };
import prenatalDiagnosticsI from './prenatal-diagnostics-i.lvl.json' assert { type: "json" };
import prenatalDiagnosticsIi from './prenatal-diagnostics-ii.lvl.json' assert { type: "json" };
import quadrupeds from './quadrupeds.lvl.json' assert { type: "json" };
import quarantineDraft from './quarantine-draft.lvl.json' assert { type: "json" };
import sickleCellAnemia from './sickle-cell-anemia.lvl.json' assert { type: "json" };
import sideEffects from './side-effects.lvl.json' assert { type: "json" };
import siteOfInfection from './site-of-infection.lvl.json' assert { type: "json" };
import situsInversus from './situs-inversus.lvl.json' assert { type: "json" };
import socialDistancing from './social-distancing.lvl.json' assert { type: "json" };
import sodiumPotassiumPump from './sodium-potassium-pump.lvl.json' assert { type: "json" };
import steadyPulse from './steady-pulse.lvl.json' assert { type: "json" };
import sterileEnvironment from './sterile-environment.lvl.json' assert { type: "json" };
import sustainedRelease from './sustained-release.lvl.json' assert { type: "json" };
import synapse from './synapse.lvl.json' assert { type: "json" };
import targetedDelivery from './targeted-delivery.lvl.json' assert { type: "json" };
import thrombosis from './thrombosis.lvl.json' assert { type: "json" };
import thymicBody from './thymic-body.lvl.json' assert { type: "json" };
import todo from './todo.lvl.json' assert { type: "json" };
import travellingSickness from './travelling-sickness.lvl.json' assert { type: "json" };
import tripleTherapy from './triple-therapy.lvl.json' assert { type: "json" };
import ulcus from './ulcus.lvl.json' assert { type: "json" };
import unnamed2 from './unnamed-2.lvl.json' assert { type: "json" };
import venaCava from './vena-cava.lvl.json' assert { type: "json" };
import ventricularSeptalDefect from './ventricular-septal-defect.lvl.json' assert { type: "json" };
import veterinaryMedicine from './veterinary-medicine.lvl.json' assert { type: "json" };
import viralLoad from './viral-load.lvl.json' assert { type: "json" };

const LEVELS : { [key: string]: LevelDefinition } = {
   allergicReaction,
   antibodies,
   anticoagulant,
   antihistaminic,
   antimicrobialResistance,
   atlas,
   autoimmuneDisease,
   axon,
   bloodBrainBarrier,
   bloodClot,
   bloodPressureRegulation,
   callostomy,
   capillaryForces,
   castleDraft,
   cellWall,
   chainReactionIi,
   chainReaction,
   circulation,
   comorbidity,
   constipation,
   corona,
   dialysis,
   diplopia,
   endoscopicSurgery,
   enzymeActivator,
   fallingSickness,
   gastricTorsion,
   headache,
   helicobacterPylori,
   influenza,
   inhibitionI,
   inhibitionIi,
   isletsOfLangerhans,
   kidneyStone,
   lockedInSyndrome,
   longTermStudy,
   lymphDrainage,
   medicationPlan,
   micellae,
   migraine,
   minimalDosageIi,
   minimalDosageIii,
   minimalDosageIv,
   minimalDosage,
   motorSkills,
   multimorbidity,
   muscleRelaxant,
   myelinSheath,
   osSacrum,
   overdose,
   pediatricDisease,
   perestalsis,
   phospholipidBilayer,
   phytomedicine,
   pitfalls,
   plateaus,
   pneumothorax,
   pons,
   prenatalDiagnosticsI,
   prenatalDiagnosticsIi,
   quadrupeds,
   quarantineDraft,
   sickleCellAnemia,
   sideEffects,
   siteOfInfection,
   situsInversus,
   socialDistancing,
   sodiumPotassiumPump,
   steadyPulse,
   sterileEnvironment,
   sustainedRelease,
   synapse,
   targetedDelivery,
   thrombosis,
   thymicBody,
   todo,
   travellingSickness,
   tripleTherapy,
   ulcus,
   unnamed2,
   venaCava,
   ventricularSeptalDefect,
   veterinaryMedicine,
   viralLoad,
}

export { LEVELS }